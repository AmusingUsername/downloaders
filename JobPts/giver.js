function getRecognitions(){
  ...
  let totalPages = 0;
  let currentPage = 0;
  const listHeaderUrl = 'https://' + appDomain + '/services/GetListOfRecognitionsForUser.xsjs?type=giver';
  const listUrl = 'https://' + appDomain + '/services/GetListOfRecognitionsForUser.xsjs?type=giver&pageParam=###&SearchParam=';
  const recognitionUrl = 'https://' + appDomain + '/services/GetRecognitionDetails.xsjs?type=giver&id=###'
  let recognitionList = [];
  let allRecognitions = [];
...
function getRecognition(url) {
  const tempRecognition = {RecognizeMessage: "", FullNameReceiver : "", TransactionDate: ""}; 
  ...
  if(xhr.status == 200){
    ...
    tempRecognition.FullNameReceiver = recognition.FullNameReceiver;
